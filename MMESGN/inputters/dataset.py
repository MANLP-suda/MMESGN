...
#torchtext.data.field  change to self_Field
...
def get_source_fields(fields=None):
  ...
  # if you want to add mask you will need in here
...
def load_fields(opt, checkpoint):
  ...
  logger.info(' * vocabulary size. source =none; target = %d' %
...
class Dataset(torchtext.data.Dataset):
  def __init__(self, fields, src_examples_iter, tgt_examples_iter,
    ...
    out_fields = [(k, fields[k]) for k in keys]#('src':data),('indices':data),('tgt':data)
    example_values = ([ex[k] for k in keys] for ex in examples_iter) #[src,indices,tgt]*sample_size
    ...
    for ex_values in example_values:
      ...
      for (name, field), val in zip(out_fields, ex_values):
        if field is not None:
          setattr(example, name, field.preprocess(val))#it sames to example. name=field.preprocess(val)
        else:
...
class self_Field(torchtext.data.Field):
  def __init__(self, sequential=True, use_vocab=True, init_token=None,
                 ...
                 is_target=False,is_numerical=True):
    super(self_Field, self).__init__( sequential, use_vocab, init_token,
  ...
  def process(self, batch, device=None):
    ...
    if self.is_numerical:
      ...
      tensor=torch.tensor(batch,dtype=self.dtype,device=device)
      if self.sequential and not self.batch_first:
        tensor.t_()
      if self.sequential:
        tensor= var.contiguous()
    return tensor
