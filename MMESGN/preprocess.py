...
def build_vocab(train_dataset_files, fields, share_vocab,
  ...
  for k in fields:#src,indices,tgt
    counter[k] = Counter()
  ...
  for _, path in enumerate(train_dataset_files):
    ...
    for ex in dataset.examples:#ex.src,ex.indices.ex.tgt
      for k in fields:
    ...
    gc.collect()
    del dataset.examples#delete the param but not clean the data
...
def parse_args():
  ...
  opts.config_opts(parser)#add the config into the opts
  opts.preprocess_opts(parser)#add  the argument needed in the project

  opt = parser.parse_args()#read the arugument
...
def build_save_vocab(train_dataset, fields, opt):
  ...
  torch.save(save_fields_to_vocab(fields), vocab_file)#return a list ,element is tuple like ('src',field)
...
def main():
  ...
  fields = get_fields() #get the dict ,it save the torchtext.data src ,target &indices without data
  # x_train, x_valid, x_test, y_train_emo, y_valid_emo, y_test_emo = data_loader.test_mosei_emotion_data()
  logger.info("Building & saving training data...")
  train_dataset_files = build_save_dataset('train', fields, opt) #shard the source retutn the shard file paths
  logger.info("Building & saving validation data...")
  build_save_dataset('valid', fields, opt)##shard the source retutn the shard file paths
